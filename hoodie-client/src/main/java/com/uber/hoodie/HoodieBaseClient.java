...
public class HoodieBaseClient implements Serializable {
  ...
  private static final Logger logger = LogManager.getLogger(HoodieBaseClient.class);
  ...
  protected HoodieBaseClient(JavaSparkContext jsc, HoodieWriteConfig clientConfig) {
  ...
  public void startEmbeddedServerView(List<String> partitions) {
    if (config.isEmbeddedTimelineServerEnabled()) {
      if (timelineServer == null) {
        ...
        timelineServer = new EmbeddedTimelineServer(jsc.hadoopConfiguration(), config);
        boolean success = timelineServer.preloadNeededPartitions(jsc, partitions);
        if (!success) {
          logger.warn("Unable to preload partitions. Proceeding as if embedded server is disabled");
          return;
        }
