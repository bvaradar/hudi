...
public class TestKafkaSource extends UtilitiesTestBase {
  ...
  public void testJsonKafkaSource() throws IOException {
    ...
    SourceFormatAdapter kafkaSource = new SourceFormatAdapter(new JsonKafkaSource(props, jsc, schemaProvider));

    // 1. Extract without any checkpoint => get all the data, respecting sourceLimit
    assertEquals(Optional.empty(), kafkaSource.fetchNewDataInAvroFormat(Optional.empty(), Long.MAX_VALUE).getKey());
    testUtils.sendMessages(TEST_TOPIC_NAME, Helpers.jsonifyRecords(dataGenerator.generateInserts("000", 1000)));
    Pair<Optional<JavaRDD<GenericRecord>>, String> fetch1 = kafkaSource.fetchNewDataInAvroFormat(Optional.empty(), 900);
    ...
    Pair<Optional<Dataset<Row>>, String> fetch2 = kafkaSource.fetchNewDataInRowFormat(
    ...
    Pair<Optional<JavaRDD<GenericRecord>>, String> fetch3 = kafkaSource.fetchNewDataInAvroFormat(
        Optional.of(fetch1.getValue()), Long.MAX_VALUE);
    ...
    Pair<Optional<JavaRDD<GenericRecord>>, String> fetch4 = kafkaSource.fetchNewDataInAvroFormat(
