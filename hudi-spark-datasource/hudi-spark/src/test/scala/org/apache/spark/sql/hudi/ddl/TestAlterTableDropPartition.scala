...
class TestAlterTableDropPartition extends HoodieSparkSqlTestBase {
  ...
  private def ensureLastCommitIncludesProperSchema(path: String, expectedSchema: Seq[String] = schemaFields): Unit = {
    ...
    val commitMetadata = HoodieCommitMetadata.fromBytes(lastInstant, metaClient.getActiveTimeline.getInstantDetails(lastInstant).get(), classOf[HoodieCommitMetadata])
    ...
    withTempDir { tmp =>
      Seq("cow", "mor").foreach { tableType =>
       ...
       """.stripMargin)
        ...
        val commitMetadata = HoodieCommitMetadata.fromBytes(lastInstant.get(), metaClient.getActiveTimeline.getInstantDetails(
