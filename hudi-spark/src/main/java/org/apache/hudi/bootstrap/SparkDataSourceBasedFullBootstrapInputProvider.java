...
public class SparkDataSourceBasedFullBootstrapInputProvider extends FullBootstrapInputProvider {
  ...
  public JavaRDD<HoodieRecord> generateInputRecordRDD(String tableName,
      List<Pair<String, List<String>>> partitionPathsWithFiles) {
    String[] partitionPaths = partitionPathsWithFiles.stream().map(Pair::getKey).toArray(String[]::new);
    Dataset inputDataset = sparkSession.read().load(partitionPaths);
    try {
