...
public class HoodieBulkInsertDataInternalWriter implements DataWriter<InternalRow> {
  ...
  public void abort() throws IOException {
    writeStatusList.forEach(ws -> {
      Path filePath =
          new Path(FSUtils.getPartitionPath(hoodieTable.getMetaClient().getBasePath(), ws.getPartitionPath()),
                  ws.getFilePath());
      try {
        hoodieTable.getMetaClient().getFs().delete(filePath);
      } catch (IOException e) {
  ...
  private void createNewHandle(String partitionPath) throws IOException {
    ...
    handle = new HoodieRowCreateHandle(hoodieTable, writeConfig, partitionPath, UUID.randomUUID().toString(),
